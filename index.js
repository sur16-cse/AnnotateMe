...
(() => {
    ...
    //Initial state of annotation tool
   ...
   // https://developer.mozilla.org/en-US/docs/Web/API/Element/contextmenu_event
    ...
    //https://blog.logrocket.com/localstorage-javascript-complete-guide/#:~:text=To%20be%20sure%20the%20browser,%2F%2F%20No%20web%20storage%20Support.%20%7D
   //https://stackoverflow.com/questions/11214404/how-to-detect-if-browser-supports-html5-local-storage
    const isSupported = () => {
        try {
            //if random value supported in local storage then bbox data will
            //only store
            const key = "__some_random_key_1234%(*^()^)___"
            //set item store data in key value pair localstorage behave like object
    ...
    if (isSupported() === true) {
        setInterval(() => {
            if (Object.keys(bboxes).length > 0) {
                // store arrays or objects,have to convert to strings.To do this, the JSON.stringify() method before passing to setItem().
    ...
    document.onreadystatechange = () => {
        //complete-e document and all sub-resources have finished loading.
    ...
    const listenCanvas = () => {
        //clientwidth-clientWidth property returns the viewable width of an element in pixels, including padding, but not the border, scrollbar or margin.
        ...
        //Canvas is constructor defined in canvas.js file
    ...
    const drawIntro = (context) => {
        ...
        //(x1, y1) and an end point of (x2, y2).
        var gradient = context.createLinearGradient(0, 0, canvas.width, 0);
       //assign colors to canvas gradient
        gradient.addColorStop("0"," magenta");
        gradient.addColorStop("0.5", "blue");
        gradient.addColorStop("1.0", "red");
        // Fill with gradient- Sets the style used when filling shapes.
        context.fillStyle = gradient;
        //Sets the style used when filling text.
    ...
    const drawNewBbox = (context) => {
        if (mouse.buttonL === true && currentClass !== null && currentBbox === null) {
            //destructuring in js to get width height of bbox
        ...
        function newFunction() {
            ...
            //mouse down to draw bboxes to position till bbox size increase
            //during mouse down 
            //mouse down =mouse.startreal value 
            //size till bbox increase = mouse.real value
    ...
    const drawX = (context, x, y, width, height) => {
        if (drawCenterX === true) {
            ...
            //from start to this postion
            context.lineTo(zoomX(centerX), zoomY(centerY + 10))
            //then make line between them
            context.stroke()
    ...
    const inside_image = (x, y, width, height, x_, y_) => {
         //check four condition two for each x or y coordinate
        //mouse real position greater than origin of image
        //mouse real position less than width of image
    ...
    const setBBoxStyles = (context, marked) => {
        ...
        //strokeStyleSets - the style for shapes' outlines.
        if (marked === false) {
            //strokestyle is for border of bbox
            context.strokeStyle = borderColor
            //fillstyle for inner background color of bbox
    ...
    const setBboxCoordinates = (x, y, width, height) => {
        //bbox first coordinate when mouse down i.e mouse start position
        //bbox coordinate 2nd coordinate is mouse last position when mouse down
        // i.e, mouse real position
        const x2 = x + width
        const y2 = y + height
    ...
    const setFontStyles = (context, marked) => {
        if (marked === false) {
         ...
         // replacing base font size with  context of zoom size of image
        context.font = context.font.replace(/\d+px/, `${zoom(fontBaseSize)}px`)
    }

     //listener for mouse different position of mouse used in this project
    ...
    const trackPointer = (event) => {
        ...
        //getBoundingClientRect() give position of top right left bottom
        ...
        if (event.type === "mousedown" && inside_image(0, 0, currentImage.width, currentImage.height, mouse.realX, mouse.realY) === true) {
           ...
           // https://stackoverflow.com/questions/56199283/how-to-catch-mouse-left-button-on-mouseover-event
             //it points right button of mouse
            if (event.which === 3) {
                mouse.buttonR = true
            } 
           //it points left button of mouse
    ...
    //after drawing every bounding box will get store here in array
    const storeNewBbox = (movedWidth, movedHeight) => {
        ...
        if (typeof bboxes[currentImage.name][currentClass] === "undefined") {
          ...
          //pushing bounding boxes in array object
        bboxes[currentImage.name][currentClass].push(bbox)

         //current bounding box property
        currentBbox = {
            bbox: bbox,
   ...
   //after resizigng bbox 
   const updateBboxAfterTransform = () => {
    if (currentBbox.resizing !== null && 
        inside_image(0, 0, currentImage.width, currentImage.height, mouse.realX, mouse.realY) === true) {
    ...
    currentBbox.moving = false
     ...
     //setting bbox marked state
     const setBboxMarkedState = () => {
        if (currentBbox === null || (currentBbox.moving === false && currentBbox.resizing === null) && inside_image(0, 0, currentImage.width, currentImage.height, mouse.realX, mouse.realY) === true) {
            const currentBboxes = bboxes[currentImage.name]
            ...
            for (let className in currentBboxes) {
                for (let i = 0; i < currentBboxes[className].length; i++) {
                    ...
                    bbox.marked = false;
                    //endX=minm of (startRealX,RealX)-bbox.width
                    const endX = bbox.x + bbox.width
                    //endX=minm of (startRealY,RealY)-bbox.height
                    const endY = bbox.y + bbox.height
                    const size = bbox.width * bbox.height

                    //check bounding box which is further creating is inside other bbox
                    if (mouse.startRealX >= bbox.x && mouse.startRealX <= endX
                        ...
                        if (size < smallestBbox) { 
                            // Make sure select the inner if it's inside a bigger one
                            smallestBbox = size
                            currentBbox = {
                                bbox: bbox,
                                index: i,
    ...
    //move bounding boxes from one location to other
    const moveBbox = () => {
        if (mouse.buttonL === true && currentBbox !== null && inside_image(0, 0, currentImage.width, currentImage.height, mouse.realX, mouse.realY) === true) {
            const endX = currentBbox.bbox.x + currentBbox.bbox.width
            const endY = currentBbox.bbox.y + currentBbox.bbox.height

            // Only if pointer inside the bbox then only bbox will move
            //initial edgeSize is minm 5
    ...
    //drag image on screen for this context is disabled above
    ...
    //zoom function to zoom the image  or bbox on basis of width of height not coordinate
    const zoom = (number) => {
        return Math.floor(number * scale)
    }

    //zoom in x direction or  for x coordinate
    const zoomX = (number) => {
        return Math.floor((number - canvasX) * scale + screenX)
    }

    //zoom in y direction or for y coordinate
    const zoomY = (number) => {
